...
//! mothers and their children. All are having great fun on the dance floor when suddenly all 
//! the lights went out. It's a dark night and no one can see each other. But you were flying 
...
function findChildren(dancingBrigade) {
    ...
    let upperCaseIndexes = [...new Set(dancingBrigade.toLowerCase().split('').sort().map((x,y,z) => z.indexOf(x)))]
...
const findChildren = (d) => d.split('').sort().sort((a, b) => a.toUpperCase() < b.toUpperCase() ? -1 : 1).join('')
  ...
  let sortedArr=dancingBrigade.split("").sort()
    ...
    for(let i=0;i<upperArr.length;i++){
        ...
        for(let j=0;j<lowerArr.length;j++){
            if(upperArr[i].toLowerCase()==lowerArr[j])
            result+=lowerArr[j]
        }
    }
    return result
